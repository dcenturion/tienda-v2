...
$(function () {
  ...
  function handleFiles(files) {
    ...
    for (var i = 0; i < fileCount; i++) {
        ...
        reader.onload = function(event) {
            ...
            $image.on('load', function() {
              showColorsForImage($image, $imageSection);
					...
					var colorThiefB = new ColorThief();
					var imageB                   = $image[0];
					var colorB                    = colorThiefB.getColor(imageB);
					var palette                  = colorThiefB.getPalette(imageB);
                    var paletteJSON = JSON.stringify(palette);
                    var colorJSON = JSON.stringify(colorB);
					

					Url = '/system/_vistas/se_configuracion.php?Main=Pantonera&pallete='+paletteJSON+'&color='+colorJSON+'';
					ValorAjaxG = FAjaxG(Url,'panelOculto',$image,$imageSection, WAjaxValidacionH);
 	...
 	function WAjaxValidacionH(e, Url,sPanel,$image, $imageSection, Form) {
