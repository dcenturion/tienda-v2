...
function P_CrearUser() {
    global $vConex, $enlace, $entidadCreadora, $ipHost, $FechaHora,$Codigo_Entidad_Usuario;
    ...
    $fecha_hora_creacion = $FechaHora;
    $usuario_creacion = $Codigo_Entidad_Usuario;
...
function P_CrearUserAlumno($AlmacenPrograma=null) {
    global $vConex, $enlace, $entidadCreadora,$Sede,$Codigo_Entidad_Usuario;
    ...
    $Escuela = post('Escuela');
    $EnvEmail = 1;#post('enviar_email');
    $IDUsuario = post('IDUsuario');
    $IdJefe = post('IdJefe');

    if(get('UsuarioEntidad') != ''){
        $Codigo = get('UsuarioEntidad');
        $sql = "SELECT * FROM usuario_entidad WHERE Codigo = $Codigo";
        $rg = fetch($sql);
        $email = $rg['Usuario'];
        $nombres = $rg['Nombres'];
        $apellidos = $rg['Apellidos'];
        $Perfil = $rg['Perfil'];
        $FechaNacimiento = $rg['FechaNacimiento'];
        $Genero = $rg['Genero'];
        $Area = $rg['Area'];
        $Cargo = $rg['Cargo'];
        $Alumno = $rg['Alumno'];
        ...
        $Sede = $rg['Sede'];
        $Escuela = $rg['Escuela'];
        $EnvEmail = $rg['enviar_email'];
        $Auxiliar = $rg['Auxiliar'];
        $IDUsuario = $rg['IDUsuario'];
        if ($usuario_creacion = $rg['usuario_creacion'] == '') {
            $usuario_creacion = $Codigo_Entidad_Usuario;
        }
    }
    ...
    if (empty($rg["Usuario"])) {
        ...
        if($EnvEmail==1){
            enviar_mail_usuario2($email, $vConex, $secure_password, $entidadCreadora,$CodAlmacen);
        }
...
function P_CrearUserProfesor() {
    global $vConex, $enlace, $entidadCreadora,$Codigo_Entidad_Usuario;
    $email = trim(post('Usuario'));
    $nombres = post('Nombres');
    $apellidos = post('Apellidos');
    $Perfil = 12;
...
function P_CrearUser_standar($vConex,$data) {
    ...
    $password = "pass2016";#GeneratePassword();
...
function enviar_mail_usuario_insc($email,$vConex,$contrasena,$Empresa){
    ...
    $sqlUser = "SELECT CodEmpresa,EmailEmisor FROM empresa WHERE PaginaWeb = '{$Empresa}' ";
    $rg = fetch($sqlUser);
    $CodEmpresa = $rg["CodEmpresa"];
	$EmailEmisor = $rg["EmailEmisor"];
    ...
    $body .= '<tr><td colspan="3" style="font-size:13px;color: #5D5D5D;"> <br>'.$Texto1.' <br>Su usuario es: <span style="color:#35A9AD">'.$usuario .'</span> y su contraseña: <span style="color:#35A9AD">'.$contrasena .'</span><br /></td></tr>';
    ...
    $emailE = emailSES3($n_a, $email, $Asunto, $body,'','',$nombre_emisor,$EmailEmisor);
    return $emailE;
}

function enviar_mail_usuario2($email,$vConex,$contrasena,$Empresa){
    ...
    $sql = "SELECT Nombres, Apellidos, Usuario, CodigoParlante FROM usuarios WHERE Usuario='$email' limit 1";
    $rg = fetch($sql);
    $n_a = ucwords(strtolower($rg["Nombres"])).' '.ucwords(strtolower($rg["Apellidos"]));
    $usuario_parlante = $rg['CodigoParlante'];
    $usuario = $rg['Usuario'];
	
    $sql2="select UrlId,Carpeta from usuarios where Usuario='$entidadCreadora'";
    $regusu=  fetch($sql2);
    $nombreEmp = $regusu['UrlId'];
	$CarpetaEmpresa = $regusu["Carpeta"];
	
	$SqlMsj = "SELECT  NombreEmpresa,EmailEmisor,asuntoEmail,emailSoporte FROM empresa WHERE PaginaWeb='{$Empresa}';";
    $msj = fetch($SqlMsj);
    $NombreEmpresa = $msj['NombreEmpresa'];
    $EmailEmisor = $msj['EmailEmisor'];
	$asuntoEmail = $msj['asuntoEmail'];     //asunto email
    $emailSoporte = $msj['emailSoporte'];   //soporte email
	if(empty($emailSoporte)){
		$emailSoporte= "soporte@proeducative.org";
    ...
    $resumen = '<br>Su usuario es: <span style="color:#35A9AD">'.$usuario_parlante .'</span> y su contraseña es: <span style="color:#35A9AD">'.$contrasena .'</span><br />';
		...
		if($tipoEnvio == 'Predeterminado' || !$CodAlmacen){
			$Texto = '<span style="color:#000">Estimado(a) <span style="color:#35A9AD">'.$n_a.'</span></span> <BR /><BR /> Su inscripción a la plataforma se realizó exitosamente.<span style="color:#35A9AD"></span>';        
			$body  = "<table border='0' width='95%' style='font-family: arial, open sans'>";
			$body .= '<tr><td colspan="3" style="padding:8px 0px;font-size:22px;" >'.$Asunto.'</td></tr>';

    
			$body .= '<tr><td colspan="3" style="font-size:13px;color: #5D5D5D;"> <br>'.$Texto.$resumen.'</td></tr>';
...
function enviar_mail_usuario_compra($email,$vConex,$contrasena,$entidadCreadora,$pagweb){
                        ...
                        $body = '
                            ...
                            <div style="width:100%;display:flex">
                                ...
                                <div style="width:80%;text-align:center;font-size:1.3em;font-family: Arial;">
                                        <div style="width:100%;text-align:center;    ">Gracias por registrarte en nuestra plataforma de lectura y aprendizaje virtual</div><br/>
                                        <div style="width:100%;text-align:center;    ">Tu Usuario es:  '.$usuario.'</div><br/>
                                        <div style="width:100%;text-align:center;    ">Tu contraseña es: '.$contrasena.'</div><br/>
    ...
    $asunto = 'Inscripción a la Plataforma Educativa';
    $emailE = emailSES($n_a,$email,$asunto, $body );
