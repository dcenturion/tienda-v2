            ...
            case "Entrevista":
                ...
                if ($TP == "Entrevista" || $TP == "Diplomado" || $TP == "Seminario") {
                    ...
                    if ($AuxEM) {
                        ...
                        $auxMsg = "El alumno " . array_shift(explode("Alumno", $usuarioId)) . " fue matriculado en el Programa {$ObjPRG->Titulo} <i class=icon-ok></i><br>";
                    }
                }
                break;
        }
        ...
        switch ($NM) {
            ...
            case "Ecomerce":
                ...
                if ($AuxEM) {
                    ...
                    $auxMsg = "El alumno " . array_shift(explode("Alumno", $usuarioId)) . " fue matriculado en el $tipoproducto {$ObjPRG->Titulo} <i class=icon-ok></i><br>";
                }
                break;
        }
    } else {
...
function update_recurso_eval($producto, $alumno, $vConex,$CursoAlmacen){
    ...
    foreach($MxCurricula as $Curricula){
        ...
        foreach($MxRE as $RE){
            ...
            $Q_TRC = " SELECT *
                    FROM elrecursoevaluacion 
                    WHERE Codigo = {$RECodigo} ";
            $RecEval =  fetchOne($Q_TRC, $vConex);

            $cod_RecEval = $RecEval->Codigo;
            $FechaInicio_RecEval = $RecEval->FechaInicio;
            $FechaFinal_RecEval = $RecEval->FechaFinal;
            $HoraInicio_RecEval = $RecEval->HoraInicio;
            $HoraFinal_RecEval = $RecEval->HoraFinal;
            $Duracion_RecEval = $RecEval->Duracion;
            ...
            if(empty($CodigoTRC)){
                ...
                insert("eltransrespuesta_cab", array(
                    ...
                    "Duracion" => $Duracion,
                    "Estado" => "Pendiente",
                    "EstadoAccesoEspecial" => "0",
                    "HoraInicioRecursoEspecial" => $HoraInicio_RecEval,
                    "HoraFinalRecursoEspecial" => $HoraFinal_RecEval,
                    "FechaInicioRecursoEspecial" => $FechaInicio_RecEval,
                    "FechaFinalRecursoEspecial" => $FechaFinal_RecEval,
                    "HoraInicioRecurso" => $HoraInicio_RecEval,
                    "HoraFinalRecurso" => $HoraFinal_RecEval,
                    "FechaInicioRecurso" => $FechaInicio_RecEval,
                    "FechaFinalRecurso" => $FechaFinal_RecEval,
                    "DuracionRecursoEspecial" => $Duracion_RecEval,
                    "TiempoToleranciaEnvioRecursoEspecial" => "00:10:00"
                ), $vConex);

                
                //REGISTRANDO EN LA TABLA TRANSRESPUESTA
                switch ($RecursoTipo) { //juanjo
...
function enviar_mail($data, $vConex,$pagweb="") {
    ...
    $sqlUsuario = "SELECT ue.Nombres, ue.Apellidos,ue.Usuario 
        FROM usuarios AS us
        INNER JOIN	 usuario_entidad	AS ue ON ue.Usuario=us.Usuario
        WHERE us.IdUsuario='{$alumno}';";

    $emal1 = explode('Alumno', $alumno);
    $destinatario = $emal1[0];
	...
	$SqlMsj    = "SELECT NombreEmpresa,EmailEmisor,asuntoEmail,emailSoporte  FROM empresa WHERE PaginaWeb='{$IdUsuarioEmpresa}';";
    $msj       = fetch($SqlMsj);
    $EmailEmisor           = $msj['EmailEmisor'];  //EmailEmisor General	
   
   $NombreEmpresa     = $msj['NombreEmpresa'];   //Nombre Empresa	
   $asuntoEmail       = $msj['asuntoEmail'];    //Asunto Email	
   if($asuntoEmail){
	   $From=$asuntoEmail; 
   }
   
   $emailSoporte      = $msj['emailSoporte'];  //Email Soporte
    ...
    $hoy = FormatFechaText($hoy);
        ...
        if($pagweb=="https://www.owlbook.org/"){
            ...
            $alumnob  = substr($alumno, 0, strlen($alumno)-6);

            $pinformation = fetch("SELECT CONCAT(Nombres,' ',Apellidos) pinformation FROM usuarios WHERE Usuario='$alumnob' ORDER BY IdCodCorrelativo DESC LIMIT 1")['pinformation'];

            $alumnob = (trim($pinformation)!="")?$pinformation:$alumnob;
             ...
             $body='
                                ...
                                <div style="width:100%;display:flex">
                                    ...
                                    <div style="width:80%;text-align:center;font-size:1.3em;font-family: Arial;">
                                            <div style="width:100%;text-align:center;    ">!Felicidades! <label style="text-decoration: underline;">'.$alumnob .'</label></div><br/>
            ...
            if($tipoEnvio == 'Predeterminado'){
                ...
                $body .= '<tr><td colspan="3" style="padding:8px 0px;font-size:22px;" >Matr&iacute;cula Exitosa</td></tr>';
                ...
                $body .= '<tr><td colspan="3" style="width:100%;padding:5px 0px;color:#6b6b6b;font-size:9px ">Datos de Soporte: Soporte '.$NombreEmpresa.' <br>  Correo: '.$emailSoporte.' <br> Tel&eacute;fono.: '.$Soporte_Telefono.' | Anexo: '.$Soporte_Anexo.'  </td></tr>';
            ...
            if($Archivo){
                ...
                $body .= "<img src='http://$dominio/system/_vistas/g_email.php?tipo=leido&CorreoUsuario=$destinatario&CodAlmacen=$CodAlmacen' width='100%'/>";
            }else{
                $body .= "<img src='http://$dominio/system/_vistas/g_email.php?tipo=leido&CorreoUsuario=$destinatario&CodAlmacen=$CodAlmacen' width='100%'/>";
            }
        }
...
function enviar_mail_dinamico($data, $vConex) {
    ...
    $TipoProducto = $objAL->TipoProducto;
        ...
        $EmailEmisor  = $msj['EmailEmisor']; //EmailEmisor General
        $NombreEmpresa = $msj['NombreEmpresa'];
        $asuntoEmail = $msj['asuntoEmail'];
        $emailSoporte = $msj['emailSoporte'];
        if(empty($emailSoporte)){
        $emailSoporte= "soporte@proeducate.org";
		 }
    ...
    if($EnviarJefe){
        ...
        $EmailJefe = "SELECT IdEmailJefe FROM usuarios WHERE Usuario = '{$emal[0]}' GROUP BY IdEmailJefe";    
        $rowJefe = fetch($EmailJefe);
        if($rowJefe['IdEmailJefe']){
           
            if(!ValidatorEmail($rowJefe['IdEmailJefe'])){ 
             $JefeCorreo = false;
            }else{
	...
	$body2 .= '  <td style="font-size:12px;border:1px solid #006ad0;padding:8px 30px;background-color:#4396de;text-decoration:none">
				  <center><a style="color:#f9f9f9;text-decoration: none;" href="mailto:soporte@proeducate.org"  target="_blank">Soporte</a></center></td>
				</td>
			   </tr>';
    ...
    $body2 .= '<tr><td colspan="3" style="width:100%;padding:5px 0px;color:#6b6b6b;font-size:9px ">Datos de Soporte:   Correo: '.$emailSoporte.' | Tel&eacute;fono.: 6440640 | Anexo: 604  </td></tr>';
    ...
    if(!ValidatorEmail($EmailEmisor)){
        $CorreoEmisor = false;
    }	
    if(!ValidatorEmail($destinatario)){
        $CorreoEmisor = false;
    }
    ...
    if($CorreoEmisor){
        if($EnviarJefe){  
            if($JefeCorreo){      
              #Usuario          
              $emailE = emailSES3($n_a, trim($destinatario),"PLATAFORMA EDUCATIVA", $body1.$bodyA.$body2,'','',$From,trim($EmailEmisor));
              #Jefe
              $destinatario = $rowJefe['IdEmailJefe'];                      
              $emailE1 = emailSES3($n_a,trim($destinatario),"PLATAFORMA EDUCATIVA", $body1.$bodyJ.$body2,'','',$From,trim($EmailEmisor));    
              $emailE = 1;
            }else{        
              $emailE = emailSES3($n_a,trim($destinatario),"PLATAFORMA EDUCATIVA", $body1.$body2,'','',$From,trim($EmailEmisor));
              $emailE = 2;
            }
        }else{
            emailSES3($n_a,trim($destinatario),"PLATAFORMA EDUCATIVA", $body1.$body2,'','',$From,trim($EmailEmisor));
            $emailE = 3;
        }        
    }else{
        $emailE = 0;        
...
function InscripcionUsuarioEntidad($data, $link_identifier) {
    ...
    if ($Escuela == '') {
        $Sql = " SELECT  Codigo From campus WHERE  Empresa = '$empresa' ";
        $Row = fetch($Sql);
        $Escuela = $Row["Codigo"];
    }
    if ($Sede == '') {
        $Sql = " SELECT SC.Codigo
                 FROM sede_sucursal  AS SC
                 INNER JOIN sedes AS SD ON SC.UnidadNegocio =  SD.Codigo
                 WHERE SD.UsuarioEntidad = '$empresa' ";
        $Row = fetch($Sql);
        $Sede = $Row["Codigo"];
    }
    if ($Perfil == '') {
