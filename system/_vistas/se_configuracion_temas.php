...
$enlace = "./_vistas/se_configuracion_temas.php";
$enlacePopup = "se_configuracion_temas.php";
...
if (get("metodo") != "") {
    ...
    if (get("TipoDato") == "texto") {
        ...
        if (get("transaccion") == "UPDATE") {
            ...
            if (get("metodo") == "formColor") {
                $inputColor = post("inputColor");
                $CodRegistro = get("CodRegistro");
				
				$reg = array($CodRegistro => $inputColor);
				$where = array('Codigo' => $Entidad);
...
function Main($Arg) {
    ...
    switch ($Arg) {
        ...
        case 'Principal':
            ...
            $titulo = "<p>COLORES DEL SITIO </p><span>Selecciona y edita</span>";
            $btn_titulo = panelST2017($titulo,"", "auto", "TituloALMB");
			
		    $reporte = htmlPantone();
			
            $html .= "<div id = 'panelOculto' style='float:left;width:100%;' ></div>";
            $html .= "<div id = 'PanelInferior' style='float:left;width:100%;background:#fff;padding-bottom:29px;' >" . $reporte . "</div>";
            $panel = array(array('PanelA1-A', '100%', $html));
            $Cuerpo = LayoutPage($panel);
			
            $titulo = "<p>COLORES DEL EMAIL</p><span>Selecciona y edita</span>";
            $btn_tituloB = panelST2017($titulo,"", "auto", "TituloALMB");
			
		    $reporteB = htmlPantoneB();
			
            $html = "<div id = 'panelOculto' style='float:left;width:100%;' ></div>";
            $html .= "<div id = 'PanelInferior' style='float:left;width:100%;background:#fff;padding-bottom:29px;' >" . $reporteB . "</div>";
            $panel = array(array('PanelA1-A', '100%', $html));
            $CuerpoB = LayoutPage($panel);			
			
            $s = layoutV2($btn_tituloP , $pestanas . $btn_titulo . $Cuerpo. $btn_tituloB . $CuerpoB); 
        ...
        case 'EditarRegistroEmailRD':
		
            $CodRegistro = get("CodRegistro");
			$html = "	
                        <script> openPopupURI('".$enlacePopup."?Main=EditarRegistroEmail&CodRegistro={$CodRegistro}', {modal:true, closeContent:null}); </script>
                    ";
            WE($html);
			
            break;     		
		
        case 'EditarRegistroEmail':
				...
				$s = "<form class='CuadroA' id='formColor' name='formColor' method='post' action='javascript:void(null);' enctype='multipart/form-data'>";
					$s .= "<ul>";
						$s .= "<li>";
								...
								$s .= "<ul >";	
									...
									$s .= "<li class='item' style='background-color:rgb(240,241,241)' 
										onclick=seleccionaComboBox('inputColor','rgb(240,241,241)');
										>rgb(240,241,241)</li>";
					...
					$viewdata = array();
					$viewdata['sUrl'] =  $enlace."?metodo=formColorEmail&transaccion=UPDATE&CodRegistro={$CodRegistro}";
...
function htmlPantoneB(){
    global $Entidad, $cnPDO, $enlace;
	
	$Query=" SELECT ColorCabeceraEmail, ColorCuerpoEmail, ColorFondoEmail 
	FROM entidades WHERE Codigo = :Entidad ";
	$rg = OwlPDO::fetchObj($Query, ["Entidad" => $Entidad ] ,$cnPDO);
	...
	$s= " <div class='Pantone'>
	        <div class='row'>
				<div class='Titulo'>
				    COLOR DE LA CABEZERA - EMAIL
				</div>	
				<div 
				     class='Color'
					 style='background-color:".$ColorCabeceraEmail."'
					 onclick=enviaReg('','{$enlace}?Main=EditarRegistroEmailRD&CodRegistro=ColorCabeceraEmail','panelOculto','');
					 >
	        ...
	        <div class='row'>
				<div class='Titulo'>
				    COLOR DEL CUERPO - EMAIL
				</div>	
				<div 
				     class='Color'
					 style='background-color:".$ColorCuerpoEmail."'
					 onclick=enviaReg('','{$enlace}?Main=EditarRegistroEmailRD&CodRegistro=ColorCuerpoEmail','panelOculto','');
					 >
	        ...
	        <div class='row'>
				...
				<div 
				     class='Color'
					 style='background-color:".$ColorFondoEmail."'
					 onclick=enviaReg('','{$enlace}?Main=EditarRegistroEmailRD&CodRegistro=ColorFondoEmail','panelOculto','');
					 >
